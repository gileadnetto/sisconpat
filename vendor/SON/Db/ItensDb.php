...
abstract class ItensDb{
  ...
  protected $table;
	...
	public function Listar()    {  
		$conn = $this->db;
		$query = "SELECT p.ID , p.PRODUTO , p.DESCRICAO , p.ID_LOCALIDADE , p.TOMBAMENTO ,DATE_FORMAT(p.DT_CAD, '%d-%m-%Y')as DT_CAD ,p.ATIVO ,p.FOTO, l.DESCRICAO as LOCAL FROM produtos p , localidade l where p.ID_LOCALIDADE = l.ID and p.ativo = 1;";
	...
	public function buscarAll($query) {
		...
		if($query != null){
			...
			$conn = $this->db;
			$query = "SELECT p.ID , p.PRODUTO, p.DESCRICAO, p.ID_LOCALIDADE, p.TOMBAMENTO, DATE_FORMAT(p.DT_CAD, '%d-%m-%Y')as DT_CAD, p.ATIVO, l.DESCRICAO as LOCAL , p.FOTO FROM produtos p INNER JOIN localidade l on l.id = p.id_localidade where (p.TOMBAMENTO like '%".$query."%' or p.PRODUTO like '%".$query."%' or p.DESCRICAO like '%".$query."%' or l.DESCRICAO like '%".$query."%') and p.ativo = 1";
	...
	public function deletar($tombamento) {
		$conn = $this->db;   
		$query = "UPDATE produtos SET ATIVO= 0 WHERE tombamento = $tombamento";
	...
	public function atualizar($item) {
		$conn = $this->db;
		$query = 'UPDATE `produtos` SET PRODUTO="'.$item['produto'].'", DESCRICAO="'.$item['descricao'].'"';
    ...
    public function atualizarLocalProduto($prod_json) {
		$conn = $this->db;	   
		$query = 'UPDATE produtos set ID_LOCALIDADE='.$prod_json['idLocalidade'].' where ID='.$prod_json['id'].';';
